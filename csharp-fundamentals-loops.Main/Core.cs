...
namespace csharp_fundamentals_loops.Main
{
    public class Core
    {
        ...
        public bool stepFour(int num)
        {
            ...
            bool existsInFavoriteNumbers = false;
            for(int i = 0; i < favouriteNumbers.Length; i++)
            {
                if (favouriteNumbers[i] == num)
                {
                    existsInFavoriteNumbers = true;
                }
            }
            return existsInFavoriteNumbers;
        }

        public bool stepFive(string hobby)
        {
            // TODO 5. Write a for loop that checks if the hobby String is in the myHobbies array
            bool existsInMyHobbies = false;
            for(int i = 0; i < myHobbies.Length; i++)
            {
                if(hobby == myHobbies[i])
                {
                    existsInMyHobbies = true;
                }
            }
            return existsInMyHobbies;
        }
    }
